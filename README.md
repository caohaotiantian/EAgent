...
| `checkpoint`  | git-backed workspace snapshots before mutating tools, with rollback. On by default; `EAGENT_CHECKPOINT=off` kill switch (the auto-snapshot runs synchronous git on every mutating call) | `/checkpoint`, `/checkpoints`, `/rollback` | — |
