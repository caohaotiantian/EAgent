...
| where refs resolve from | `--extension-module` — `resolver.register(r)`, a `ResourceResolver {resolve, document?, subgraph?}` — the REQUIRED member and only it, since demanding `document` refused a resolver implementing exactly the published interface. SUBSTITUTES rather than layers: a module supplying one owns ref resolution for the deployment, `resources/` included | a second claim refuses: `registers a resolver, and <first module> already registered one` |
