...
| **Approval modes** | **There are none, and `approval` declares two fields: `approvers` and `separationOfDuties`.** `mode`, `k` and `delegation` were deleted — writing any of them is `GRAPH020_UNKNOWN_FIELD`. **k-of-n approval needs no new vocabulary:** N `human_gate` nodes joined by `join{branches:[…], mode:"quorum", k}` — see `examples/graphs/two-person-approval.json`, **whose `onBranchError: "fail"` makes it two-of-three WITH A VETO rather than k-of-n tolerance of a dissenter**: `"fail"` is read before `k` ever matters, so a rejection arriving before the second approval fails the run whatever the other two would have said (after it, the run still fails and the write has already landed). `onBranchError: "skip"` is now the honest alternative and the example does not yet take it: since §A.75 `k` is a floor the FOLD enforces, so measured on that graph under `skip` one approval of three is refused at the barrier (`E_QUORUM_UNREACHABLE`, no write) in all three orderings. **Both modes refuse a run below `k` and they differ in WHEN** — `"fail"` on the first rejection, before `k` matters; `"skip"` at quiescence, so a lone dissenter does not veto. **Which one the example should teach was DECIDED on 2026-09-22: quorum** — the canonical file becomes `"skip"` and veto moves to a second graph keeping `"fail"`, whose description will say that the first reject decides and that a late reject cannot recover a write that already landed (`DESIGN.md` D9, `TODO.md` §A.68). **It still ships `"fail"` today**: the decision is recorded and the file changes with the implementation, so everything said above describes the graph as it is now. `tiered` was not implementable from its own declaration, because no field anywhere defines a tier; delegation presupposes a group vocabulary this system does not have. Residue: a short-circuiting quorum join leaves the unneeded gates OPEN |
