...
```bash
git clone -b loom https://github.com/caohaotiantian/EAgent && cd EAgent
...
trace. `pack.mjs` packs the COMMITTED tree — `git archive HEAD`, compiled in a throwaway checkout, so
uncommitted, staged or untracked files cannot ship; it prints the `branch@sha` it packed and how many
local changes it left out, and strips the `sourceMappingURL` pointers to maps it does not ship — and
...
| **Gates** | `npm run check` — 3,500+ tests, offline, no API key; three guards: zero-dep, public surface (the exported NAME SET, in `scripts/surface.json`) and the kernel file list (`scripts/kernel.json`) |
...
| | |
|---|---|
| **A verdict from a function or evaluator body** | **Works, and there are exactly two verdicts, both through the RETURN** — `OUTCOME_KEYS` is `{writes, take, retry, refuse}` and the first two are the ordinary value. To be retried, a body returns `{ retry: { reason } }` and the engine raises `E_FUNCTION_UNAVAILABLE` on its behalf, which is retryable by class. To fail ON PURPOSE it returns `{ refuse: { reason } }` and the engine raises `validation`/`E_FUNCTION_REFUSED`, which is **never** retried whatever the node's `retry` policy says — `validation` is not in `RETRYABLE`, so the retry decision returns before it reads a policy. A *throw* still carries neither: `isLoomError` is an `instanceof` against the host class and a guest object can never satisfy it, so every throw out of the `vm` is `E_INTERNAL` — now the code for a BUG rather than the code for both. `E_FUNCTION_REFUSED` was a kernel change and carries a `Kernel-seam:` trailer (`f7f74d5`) |
| **Reading the clock in a body** | **Reproducible.** `ctx.now()` is the task's journaled lease timestamp, so replay computes the same number with nothing new written, and time does not advance during a task. `Date` is **bound to the same number**: `new Date()`, `Date()` and `Date.now()` all read `ctx.now`, and `new Date(x)` forwards to the real one — a frozen `Date` that silently never advances, or a second clock disagreeing with the first, are both worse than one clock. It throws `E_EFFECT_UNRECORDED` until a call seeds it. **Hook bodies still get no `Date`**, because `HookContext` has no `now` to bind it to |
...
| **Approval modes** | **There are none, and `approval` declares two fields: `approvers` and `separationOfDuties`.** `mode`, `k` and `delegation` were deleted — writing any of them is `GRAPH020_UNKNOWN_FIELD`. **k-of-n approval needs no new vocabulary:** N `human_gate` nodes joined by `join{branches:[…], mode:"quorum", k}` — and it ships as TWO files, because there are two products and one file must not teach both (`DESIGN.md` D9). **The canonical `examples/graphs/two-person-approval.json` is QUORUM** — `onBranchError: "skip"`, so a lone dissenter is outvoted and two approvals land the write whenever the third vote arrives; below `k` nothing is written, because since §A.75 `k` is a floor the FOLD enforces and one approval of three is refused at the barrier (`E_QUORUM_UNREACHABLE`) once every gate is answered. **`examples/graphs/two-person-veto.json` is VETO** — the same graph with `onBranchError: "fail"`, read before `k` ever matters, so the first reject decides (`E_HUMAN_APPROVAL_REQUIRED`) whatever the other two say; and its description states the product limit: a late reject, arriving after the second approval, fails a run whose write has ALREADY LANDED. The failed run's rollback then runs `fs.restore`, which puts back a file that existed before the run and REMOVES one the write created — so the late reject undoes the write in both cases, though the directory `fs.write` created for it (`approved/`) is left behind, empty (§A.116) — and refuses, journaling a FAILED compensation, if the file's bytes changed after the write rather than delete bytes the run did not produce (`TODO.md` §A.99; `loom trace` still draws that failed undo `[ok]`, so read the journal's `compensation.recorded`, §A.100). **Both refuse a run below `k`, and both mark a run failed only once every gate is answered; they differ in WHICH runs they refuse** — `"skip"` only a run below `k`, `"fail"` any run with a rejection in it, including two approvals over one dissenter. `packages/core/test/graph/two-person-approval.test.ts` drives both. `tiered` was not implementable from its own declaration, because no field anywhere defines a tier; delegation presupposes a group vocabulary this system does not have. Residue: a short-circuiting quorum join leaves the unneeded gates OPEN |
...
loom gates <runId>                    # the gate id, and the channels the gate node reads
...
Two things that walkthrough used to do to you no longer do. The `gate … — loom approve …` hint is
...
and `--max-bytes 0` removes it (§A.51, `5d2053b`). **To learn WHICH values were cut, ask the row —
...
| a `function` node body | `resources/function/*.js`, `*.mjs` — a bare function expression. It is handed `ctx.node`, its own node as the graph declared it (`{id, type, reads, writes, out}`), so a bound the graph already states is read rather than spelled twice; and it may return `{refuse: {reason}}` to fail on purpose under its own code | `examples/README.md` §2; `examples/resources/function/triage-plan.js` does both |
...
scripts/           CI guards, the binary build, and the tarball pack and install smoke
```
