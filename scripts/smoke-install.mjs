 ...
 * --prefix` of it into a private global root. BINARY MODE: a copy of the SEA binary in the temp
 * directory — out of its `bin/`, so it runs as a shipped copy with no sources beside it. Both then:
 *
...
if (mode === "tarball") {
  ...
  looms.push(["local", join(project, "node_modules", ".bin", process.platform === "win32" ? "loom.cmd" : "loom")]);
  ...
  looms.push(["global", process.platform === "win32" ? join(prefix, "loom.cmd") : join(prefix, "bin", "loom")]);
} else {
...
writeFileSync(join(ws, "input.txt"), "hello\n"); // README: `echo hello > input.txt`
{
  ...
  check("README copy.json: loom compile", c.code === 0, `exit=${String(c.code)} ${oneLine(c.out + c.err)}`);
  const r = run(loom, ["run", "graphs/copy.json", "--input", readmeInput("graphs/copy.json")], ws);
  const out = json(r.out);
  check("README copy.json: loom run", r.code === 0 && out?.status === "succeeded", `exit=${String(r.code)} status=${String(out?.status)}`);
  let copied = "";
  try {
  ...
  check("README copy.json: out/copy.txt equals input.txt", copied === "hello\n", JSON.stringify(copied));
  if (typeof out?.runId === "string") {
    const rp = run(loom, ["replay", out.runId], ws);
    const verdict = json(rp.out);
    check(`README copy.json: loom replay ${out.runId}`, rp.code === 0 && verdict?.match === true, `exit=${String(rp.code)} match=${String(verdict?.match)}`);
  } else {
    check("README copy.json: loom replay <runId>", false, "the run printed no runId");
  }
}
...
writeFileSync(join(ws, "graphs", "gated.json"), readmeGraph("graphs/gated.json"));
{
  ...
  check("README gated.json: loom compile", c.code === 0, `exit=${String(c.code)} ${oneLine(c.out + c.err)}`);
  const r = run(loom, ["run", "graphs/gated.json", "--input", readmeInput("graphs/gated.json")], ws);
  const out = json(r.out);
  const shipped = join(ws, "shipped.txt");
  check(
    "README gated.json: loom run stops at the gate with nothing written",
  ...
  if (check("README gated.json: the run prints the approve command", m !== null, m === null ? oneLine(r.out + r.err) : m[0])) {
    ...
    check("README gated.json: loom approve --as u:alice lands the write", a.code === 0 && body === "ship it", `exit=${String(a.code)} shipped.txt=${JSON.stringify(body)}`);
  }
}
