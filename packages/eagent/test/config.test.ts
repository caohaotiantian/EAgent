...
import { LayeredConfig, configEnvName, isSecretKey, loadConfigFile } from "../src/config.ts";
...
test("envOnlyConfig fallback: an un-configured host honors env and defaults", async () => {
  ...
  const { ExtensionHost } = await import("../src/kernel/extension.ts");
  const { Agent } = await import("../src/kernel/agent.ts");
