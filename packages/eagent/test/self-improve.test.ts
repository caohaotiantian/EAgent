...
import type { ToolResultBlock, UI } from "../src/kernel/types.ts";
import { makeHarness, type Harness } from "./helpers.ts";
import { BUILTIN_EXTENSIONS } from "../src/host.ts";
import selfImprove, {
  EVAL_RUNNER_PATH,
  hashFixtures,
  parseScorecard,
  ...
  setSpawn,
  vetoCandidate,
} from "../src/extensions/self-improve.ts";
...
test("runScored assembles the import.meta.url runner command and parses the scorecard (AC-D6)", async () => {
  ...
  assert.match(captured, /^node \//, "the command spawns node directly — Node 24 type stripping replaced the tsx loader");
