...
/** Resolved from THIS file, not from `process.cwd()`: the suite runs from the monorepo root. */
const DEFINE_PATH = JSON.stringify(
  join(dirname(fileURLToPath(import.meta.url)), "..", "src/kernel/define.ts"),
);
