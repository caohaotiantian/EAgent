...
test("the kernel source stays small", async () => {
  ...
  const dir = join(PKG_ROOT, "src", "kernel");
  let lines = 0;
  ...
  // CEILING RAISED 2335 -> 2342 ON THE MOVE INTO THE MONOREPO, by exactly the mechanical cost
  // and no more. Loom's tsconfig sets `erasableSyntaxOnly`, which forbids TypeScript parameter
  // properties, so `constructor(private readonly x: T) {}` becomes a field declaration plus an
