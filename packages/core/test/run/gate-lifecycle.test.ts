...
test("REWIND DOES NOT REPLAY PAST A HUMAN'S REJECTION", async () => {
  ...
  await assert.rejects(() => rewindWithPlan(h.engine, runId, at, "try again"), conflict(CODES.E_RESTORE_ILLEGAL));
...
test("A REJECTION CAN NEVER BE INSIDE A SUPPRESSED RANGE — which is why the scan need not filter one", async () => {
  ...
  for (let target = 0; target <= Number(head) + 1; target++) {
    await rewindWithPlan(h.engine, runId, target as Seq, "sweep").catch(() => undefined);
  }
...
test("…but rewinding past an APPROVAL is allowed, because it ASKS AGAIN rather than assuming", async () => {
  ...
  const rewound = await rewindWithPlan(h.engine, runId, at, "re-run the write");
...
test("A REWIND TO SEQ 0 IS REFUSED, because it ERASES the run rather than rewinding it", async () => {
  ...
  await assert.rejects(() => rewindWithPlan(h.engine, runId, 0 as Seq, "start over"), conflict(CODES.E_RESTORE_ILLEGAL));
  await assert.rejects(() => rewindWithPlan(h.engine, runId, -1 as Seq, "start over"), conflict(CODES.E_RESTORE_ILLEGAL));
...
test("…and seq 1 is the floor, not a synonym for it: a run keeps its identity there", async () => {
  ...
  const rewound = await rewindWithPlan(h.engine, runId, 1 as Seq, "back to the submission");
...
test("A REWIND ONTO A GATE'S EXPIRY IS REFUSED, for the same reason as one onto its decision", async () => {
  ...
  await assert.rejects(
    () => rewindWithPlan(h.engine, runId, timeout.seq, "undo the expiry"),
  ...
  const reopened = await rewindWithPlan(h.engine, runId, (timeout.seq - 1) as Seq, "ask again");
...
test("…and a gate.timeout that is NOT an expiry is still a legal boundary", async () => {
  ...
  const rewound = await rewindWithPlan(h.engine, runId, marker, "undo the clock's no-op");
