...
test("THE `todo` LIST MUST NOT GROW", () => {
  ...
  assert.ok(todos <= 7, `${todos} event types are marked todo; it was 15 when this gate was written, 7 now, and may only shrink`);
  assert.ok(AUDIT_RULES.length >= 16, `${AUDIT_RULES.length} rules; deleting one needs a reason in the journal, not a quiet edit`);
});
