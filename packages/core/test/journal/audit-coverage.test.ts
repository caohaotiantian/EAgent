...
const EXCUSED: Readonly<Record<string, { readonly kind: Excuse; readonly why: string }>> = {
  ...
  "policy.decided": { kind: "todo", why: "a gate.raised should be preceded by a policy.decided that gated it — the missing half of the oversight record" },
  ...
  "subgraph.started": { kind: "todo", why: "should reach subgraph.completed on a run that completed, and the child runId should be derivable" },
  "subgraph.completed": { kind: "todo", why: "the mirror: a completion with no start means a child nobody recorded" },
  "hook.applied": { kind: "todo", why: "the ref should appear in the graph's declared hooks for that point — an unlisted extension rewrote something" },
};
...
test("THE `todo` LIST MUST NOT GROW", () => {
  ...
  assert.ok(todos <= 11, `${todos} event types are marked todo; it was 15 when this gate was written, 11 now, and may only shrink`);
  assert.ok(AUDIT_RULES.length >= 11, `${AUDIT_RULES.length} rules; deleting one needs a reason in the journal, not a quiet edit`);
});
