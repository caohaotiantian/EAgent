...
// `openConsole`/`Page` — the console's own script, running against a live plane in a mocked
// DOM — moved to `./console-page.ts` so `console.test.ts`'s SSE-fold tests (A.45) can drive the
// same harness instead of carrying a second hand-rolled copy that silently diverges. See that
// file's header for what is and is not under test.
