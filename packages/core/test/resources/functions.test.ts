...
test("the seed is NOT visible to the body — the exact ctx key set for a node declaring no effects", () => {
  ...
  //
  // A NODE DECLARING `effects` SEES FIVE KEYS, and the title says so rather than the assertion
  // quietly covering less than it claims — `ARGUMENT_BRIDGE` adds `ctx.effects` (a throwing stub
  // in a realm) only when `p.declaredEffects` is non-empty. The test below drives no effects, so
  // the exact set it pins is the four-key one. `A SANDBOXED BODY IS TOLD WHY IT CANNOT INVOKE A
  // DECLARED EFFECT`, further down this file, is the fifth key's own test.
