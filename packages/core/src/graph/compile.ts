...
export function compile(input: CompileInput): CompileResult {
  ...
  // `unknownEdgeKinds` USED TO BE PREPENDED HERE, and the relocation is §A.80. The check lives in
  // `graph/validate.ts`'s `checkStructure` now, beside `GRAPH020_UNKNOWN_FIELD` — which is where
  // its own docstring always said it belonged, and, more than tidiness, the only place that
  // reaches a subgraph CHILD: `rule016Subgraphs` recurses `validateGraph`, not `compile`, so the
  // one refusal that is total over `kind` stopped at the top-level spec and `kind: 42` in a child
  // compiled clean.
  const diagnostics = [...validateGraph({ ...input, depth: 0, expanding: [], index })];
