 ...
 *
 * NO WARNING HERE, deliberately, and that is not an inconsistency with the two sites that emit
 * one. This refusal reaches the operator as a `task.committed` failure carrying the reason and
 * the child's id — journalled evidence, which is strictly better than stderr. The warning exists
 * at the other two sites because a swallow leaves NO evidence anywhere.
 *
 * `retryable` IS THE GRAPH'S CALL, NOT THIS FUNCTION'S. A node with no `retry` policy still fails
 * immediately; the class only says the failure is one a retry could clear.
 */
function childUnavailable(childRunId: RunId, what: string, e: unknown): LoomError {
  const why = describeThrown(e);
  return err.unavailable(CODES.E_SUBGRAPH_FAILED, `${what} for child run ${String(childRunId)}: ${why}`, {
...
/**
 * What a run's own journal says it was compiled and submitted under. See `#compiledIdentity`.
 *
 * The last three are OPTIONAL because every journal written before they existed lacks them, and
 * each says what absent means at its own declaration in `journal/events.ts`. None of them may be
 * defaulted to a permitting value here: `#assertBound` falls back to the aggregate floor check it
 * always had, and `restore` leaves the deployment's own ceilings standing.
 */
interface RecordedIdentity {
  readonly graphHash: string;
  readonly manifest: string;
  /** The `max` over every node's compiled floor, from `run.started`. */
  readonly posture: Posture;
  /** The PER-NODE compiled floors, from `run.compiled.postures`. */
  readonly postures?: ReadonlyMap<NodeId, Posture>;
...
interface RunContext {
  ...
  /**
   * Nodes a BRANCH DECISION selected with a choice made from untrusted content. E8's other half.
   *
   ...
   * the argument for the region being the branch arm rather than everything downstream, and
   * `choiceOf` names the four mechanisms that make a decision — a router is only one.
   *
   ...
   *
   * NOT `readonly`: `#advanceSerially` narrows it once per attach from the bound recorded on
   * this run's own `run.submitted`, because a process that did not start the delegation has no
   * parent to inherit from. It only ever narrows — the same filter `#contextFor` applies.
   */
  grantBound: readonly string[] | undefined;
  /**
...
export class Engine {
  ...
  /** Runs THIS engine retired, to the graph hash they retired with. See `#retire` and `#retainedGraphOf`. */
  readonly #retiredRuns = new Map<RunId, string>();
   ...
   *
   * THIS IS THE ONE CROSS-RUN SITE THAT ESCAPED THE VERB. Everything else that reaches into a
   * child from `#runSubgraph` runs under `#runWave`'s `try/catch`, which turns a throw into a
   * failed task; this runs inside `#failRun`'s compensation walk, which does not. Measured at
   ...
   *
   * FAIL CLOSED IS AN EMPTY SHARE OF THE WALK: this child contributes no steps, the parent's own
  ...
  async #planRollbackChild(
    ...
    try {
      ...
      // `describeThrown`, not the bare idiom, for the reason its own docstring gives: the thrower
      // is a third-party `StateStore` that owes nobody an `Error`, and a guard whose failure path
      // can fail is not a guard.
      const why = describeThrown(e);
      process.emitWarning(
        `could not plan the undo of child run ${String(child.runId)} from "${child.ref}": ${why}; ` +
          `its effects are left as they are, the rest of this rollback still runs, and a later rewind will try again`,
        { code: "LOOM_ROLLBACK_CHILD_UNREADABLE", detail: JSON.stringify({ childRunId: child.runId, ref: child.ref, error: why }) },
      );
      return [];
    }
  }
  ...
  async #restoreEvidence(ctx: RunContext): Promise<void> {
    for await (const ev of this.#store.read(ctx.runId, 1 as Seq)) {
      ...
      // each descendant's decision. Folded AFTER `applyTaint` on the same event so the ordering
    ...
    await ctx.log.append([
      {
        ...
        payload: {
          ...
          // THE TWO BOUNDS A DECISION READS, WRITTEN WHERE A FOLD CAN REACH THEM. Both were
          // arguments to `#contextFor` and lived in one process's `PolicyEngine`, which is
          // CLAUDE.md's first non-negotiable exactly. The child of a `subgraph` node is where
      ...
      },
      {
        ...
        payload: {
          ...
          // THE PER-NODE FLOOR, SORTED. `graphHash` excludes `plans` by design, so the value
  ...
  async #advanceSerially(runId: RunId): Promise<RunProjection> {
    ...
    for (;;) {
      ...
      // else from a projection taken before it. It cannot spin: the mirror it answered is no
      // longer `open`, so the next pass writes nothing.
      if (await this.#answerMirrorsTheChildAlreadyDecided(ctx, p)) continue;
  ...
  async #forwardToParentMirrors(runId: RunId, p: RunProjection, checked?: Set<GateId>): Promise<void> {
    ...
    for (const gate of decided) {
      // MARKED WHEN THE WORK IS DONE, NEVER BEFORE IT IS TRIED. Marking the whole batch up front
      // made the set a record of what had been LOOKED AT rather than what had been ANSWERED, and
      // the two differ on exactly the path that matters: a mirror this pass could not resolve was
      // never retried by a later one, so the parent stayed `awaiting_gate` on an `open` mirror —
      // the defect this method exists to close, restored under contention. Only a restart
      // re-derived it, and a live `loom serve` does not restart.
      //
      // NO MIRROR RECORD AT ALL IS NOT AN ANSWER EITHER. The parent raises a mirror of the gate
      // the child is WAITING on, so the common shape is that it never mirrors this one and there
      // is nothing to do — but "has not raised it yet" and "will never raise it" are the same
      // absence here, and only one of them is finished. Left unmarked so the next pass asks
      // again; the cost is one parent fold per advance of a child whose gates the parent does not
      // mirror, which is the fold this set was introduced to avoid and not one anybody waits on.
      ...
      try {
        ...
        // (`MAX_APPEND_RETRIES`), so one reaching here means the retries were EXHAUSTED — a
        // transient failure under contention, not an idempotent no-op. Swallowed so the child's
        // own advance is not failed by the parent's contention, and deliberately left UNMARKED so
        // the next pass tries again. This is the one case where marking and swallowing together
        // would strand the parent for the life of the process.
        if (e.code === CODES.E_SEQ_CONFLICT) continue;
        throw e;
      }
    }
  }
  ...
  /**
   * Answer THIS run's open mirrors whose child gate is already decided — the other half of
   * `#forwardToParentMirrors`, and neither half covers the other's gap.
   *
   * THE GAP IS A RACE WITH A COMMIT WINDOW. `#runSubgraph` decides to raise a mirror while the
   * body runs; the `gate.raised` is not appended until the whole wave commits. A human who
   * answers the child's own gate inside that window is forwarded by the child — which finds no
   * mirror yet, because there is none — and the mirror then lands OPEN on a gate that is already
   * decided. Reproduced deterministically with a slow sibling task holding the wave open:
   ...
   * parent is never waiting on one. The child run id is derived the way `#runSubgraph` derives it
   * (`${runId}~${taskId}`), so this reads the same run the mirror was raised for and no other.
   *
   * EVERY OTHER TERM IS `#forwardToParentMirrors`'s, deliberately: `approve` only, on the same
   ...
   * refusing: `#forwardToParentMirrors` adds a gate to `mirrorsChecked` only AFTER that gate is
   * genuinely answered, so whatever it did not reach is retried by the next pass. It marks as it
   * goes, gate by gate — a throw on the third of five leaves the first two marked, which is right,
   * because those two WERE answered — so the claim is "nothing unanswered is recorded as
   * answered", not "a failed pass marks nothing". The retired path keeps no memo at all, so it
   * retries every decided gate on any later touch.
   *
   ...
   * across a BRANCH DECISION choosing that action's arm from tainted data, which this sentence
   * used to name as the gap and which was a live injection path to an irreversible action while
   * it did. Control-flow taint is `ctx.controlTainted`, folded by `applyControlTaint`; it reaches
  ...
  async deescalate(
    ...
    // REFUSE FIRST, JOURNAL SECOND, MUTATE THIRD, and the order is the whole of this method.
    //
    // It used to mutate and then append, so a failed append — a stale fencing token, a full
    // disk, eight exhausted seq-conflict retries — left this run executing at a lowered posture
    // that no journal records, with the caller holding an error and every reason to believe
    // nothing happened. `restore` could not clean it up either: it only ever ADDED ceiling keys.
    // Measured on a run past its first advance, so the re-seed was not there to hide it: the
    // node behind the gate ran with no gate of its own.
    //
    // The refusals still run before the append, or the journal would carry a de-escalation that
    // was going to be refused — which is `assertMayDeescalate`'s reason for existing.
    ctx.policy.assertMayDeescalate(scope, justification, actor);
    // FROM THE FOLD, not from the live object. The journaled `from` is an audit fact, so it has
  ...
  #retire(runId: RunId): void {
    ...
    // AND WHICH RUNS THOSE WERE, keyed by RUN, because "did this run finish here?" is a question
    // about a run and `#graphs` cannot answer it: one graph is shared by every run that names it,
    // so a hash lookup says yes for a run this engine never saw the moment any sibling of it
    // retires. That difference is load-bearing — `#retiredHere` gates the door that answers a
    // redelivered gate decision from the journal instead of `E_RUN_NOT_FOUND`, and
    // `cancel-cascade.test.ts` pins the refusal for a run this engine never attached. Measured
    // before this map existed: engine A, having retired its OWN run of graph G, answered a gate
    // decision for a DIFFERENT run of G it had never seen — `E_GATE_ALREADY_RESOLVED` where a
    // fresh engine said `E_RUN_NOT_FOUND`.
    //
    // BOUNDED THE SAME WAY AND FOR THE SAME REASON, oldest out. Past the bound a rewind of an old
    // run needs an explicit `attach`, which a restart needs anyway.
    this.#retiredRuns.delete(runId);
    this.#retiredRuns.set(runId, graph.graphHash);
    if (this.#retiredRuns.size > RETAINED_GRAPHS) {
      const oldest = this.#retiredRuns.keys().next().value;
      if (oldest !== undefined) this.#retiredRuns.delete(oldest);
    }
    this.#runs.delete(runId);
  }
  ...
  /**
   * The graph `#retire` kept FOR THIS RUN, unless `forget` said not to: what `#reattach`
   * re-attaches.
   *
   * `#retiredRuns` is asked first and is what makes this a question about the run rather than
   * about its graph; the journal's `compiledGraphHash` is then checked against it, so a run whose
   * journal names a different compile than the one this engine retired is not re-attached with
   * the wrong graph.
   */
  async #retainedGraphOf(runId: RunId): Promise<RunGraph | undefined> {
    if (this.#forgotten.has(runId)) return undefined;
    const retired = this.#retiredRuns.get(runId);
    if (retired === undefined) return undefined;
    const hash = await this.compiledGraphHash(runId);
    return hash === retired ? this.#graphs.get(hash) : undefined;
  }
  ...
  async planRewind(runId: RunId, atSeq: Seq, by: HumanActor): Promise<RewindPlan> {
    ...
    try {
      ...
      // A PREVIEW RELEASES WHAT IT INSTALLED. This verb is the read half of `rewind` and must
      // leave the engine holding exactly what it held before — otherwise an operator previewing
      // rewinds of finished runs re-attaches one per call, for the life of the process, which is
  ...
  async #rewindRefusals(
    ...
    readonly p: RunProjection;
    readonly live: RunContext | undefined;
    readonly ctx: { readonly log: RunLog };
    readonly attachedHere: boolean;
  }> {
    ...
    //
    // AND NOTHING IS RE-ATTACHED UNTIL THE REFUSALS HAVE PASSED. `#reattach` calls `attach`,
    // which enters the run into `#runs` — where only a terminal exit takes it out again, and a
    // finished or cancelled run has no terminal exit left to reach. Doing it above the refusals
    // meant that `planRewind`, whose whole contract is that it is the READ half of the verb, and
    // every `rewind` that was then refused, each re-attached a retired run permanently: the
    // graph, the branch index, the taint sets and the expression cache that item 7 exists to
    // release, re-installed by a preview. Measured with a spy on `attach`: one call per
    // `planRewind` of a finished run and one per `E_RESTORE_ILLEGAL` refusal on a cancelled one.
    // Every refusal below reads the fold or the log, and neither needs a context.
    const held = this.#runs.get(runId);
    const ctx = { log: held?.log ?? this.#logFor(runId) };
    ...
    // EVERY REFUSAL HAS PASSED, so a context is now worth installing. `attachedHere` is what the
    // caller needs to undo it: `planRewind` is a preview and releases what it caused, `rewind`
    // keeps it because dispatching compensations needs the graph, the policy engine and the abort
    // signal — none of which are in the journal.
    const live = held ?? (await this.#reattach(runId));
    // ONE LOG PER RUN when there is a context, which is what this was before the attach moved:
    // two `RunLog`s over one run keep two cached heads, and `commit` does not retry a stale one.
    return { p, live, ctx: live === undefined ? ctx : { log: live.log }, attachedHere: held === undefined && live !== undefined };
  }
   ...
   */
  async #compiledIdentity(runId: RunId): Promise<RecordedIdentity | undefined> {
    let graphHash: string | undefined;
    let manifest = "";
    let posture: Posture | undefined;
    let postures: ReadonlyMap<NodeId, Posture> | undefined;
    let limits: BudgetLimits | undefined;
    let capabilities: readonly string[] | undefined;
    for await (const ev of this.#store.read(runId, 1)) {
      if (isEvent(ev, "run.submitted")) {
        // THE BOUNDS THIS RUN WAS SUBMITTED UNDER. They were `#contextFor` arguments and
        // nothing else, so a fresh process rebuilt a subgraph child at the DEPLOYMENT's budget
        // and the child's own declared capabilities — see `run.submitted.limits`.
        limits = ev.payload.limits;
        capabilities = ev.payload.capabilities;
      } else if (isEvent(ev, "run.compiled")) {
        graphHash = ev.payload.graphHash;
        manifest = manifestKey(ev.payload.resolutionManifest);
        const rows = ev.payload.postures;
        if (rows !== undefined) postures = new Map(rows.map((r) => [r.nodeId, r.posture]));
  ...
  async #assertBound(ctx: RunContext, why: string, opts: { readonly requireRecord: boolean } = { requireRecord: true }): Promise<void> {
    ...
    //
    // PER NODE FIRST, because the aggregate cannot see a per-node drop. `run.started.posture` is
    // a `max` over every node, so a graph holding `in` at a `human_gate` reports `in` however far
    // one tool node's own floor falls — measured end to end: same journal, same graphHash, a
    // restart into a process registering `chat.post` as `read_only`, and the externally-visible
    // post fired with no gate. The recorded map is checked first and names the node; the
    // aggregate stays as the answer for journals written before the map existed.
    const recordedPostures = recorded.postures;
    if (recordedPostures !== undefined) {
      for (const [nodeId, was] of recordedPostures) {
        // A node the attached graph no longer HAS is not a loosening on this axis: the spec is
        // bound by `graphHash` above, so reaching here means the node exists or the graph is a
        // recorded successor. `undefined` still fails closed — `isLoosening` reads `"out"` for
        // it, which is a drop from any recorded floor above `out`.
        const nowAt = ctx.graph.plans[nodeId]?.posture ?? "out";
        if (!isLoosening(was, nowAt)) continue;
        throw err.policy(
          CODES.E_GRAPH_MISMATCH,
          `run ${ctx.runId} compiled node "${nodeId}" under oversight "${was}" and this process computes "${nowAt}" for it — ` +
            `usually a tool the original process had registered and this one does not`,
          { details: { runId: ctx.runId, nodeId, recorded: was, computed: nowAt } },
        );
      }
    }
  ...
  async #executeTask(ctx: RunContext, leased: RunProjection, w: Wave): Promise<NodeOutcome> {
    ...
    //
    // AND THE OTHER WAY UNTRUSTED CONTENT REACHES AN ACTION: it CHOSE the action. A branch
    // decision made on an injected channel selects a node that reads nothing untrusted itself,
    // so the line above sees a clean read set and E8 stayed silent while an irreversible action
    // ran under a de-escalated ceiling. Same rule, same code, same floor — the evidence is a
    // branch decision instead of a read, so the detail says `selectedBy` and names the node that
    // decided. `applyControlTaint` computes the membership at THAT node's commit; see it for why
    // the marked set is the branch arm and not everything downstream, and `choiceOf` for the
    // four mechanisms that narrow a `take` — a `router` is only the first.
    ...
    if ((tainted || chosenByTaint !== undefined) && isHardToUndo(irreversibility)) {
      this.#escalate(ctx, "taint", node.id, {
        ...
        ...(chosenByTaint === undefined
          ? {}
          : { selectedBy: chosenByTaint.decidedBy, selectedFrom: chosenByTaint.channels }),
      });
    }
  ...
  async #runSubgraph(ctx: RunContext, p: RunProjection, w: Wave): Promise<NodeOutcome> {
    ...
    //
    // AND A GRAPH FIELD MAY NOT RAISE AN OPERATOR'S CEILING. `budgetShare` is a plain number in
    // a spec that `graph/validate.ts` does not check at all, and this multiplication was handed
    // straight to `#contextFor` as the child's `runUsd` — while `#contextFor` returns an
    // EXISTING context untouched, so `submit`'s min-fold (there precisely "so `min` cannot
    // silently raise the deployment's ceiling") arrived after the ceiling was already built.
    // Measured on a $10 deployment with `budgetShare: 1000`: the child ran at $10,000.
    //
    // CLAMPED, THEN MIN-FOLDED, and both halves are load-bearing. A share outside `(0, 1]` is
    // not a share, so it clamps to nothing rather than to something: `-3` journaled a ceiling of
    // `-30` and `"abc"` produced `runUsd: NaN`, against which `committed + estimate > NaN` is
    // false for EVERY input — a dollar ceiling switched off rather than exceeded, caught today
    // only by the canonicalizer refusing a non-finite number in an unrelated layer. The
    // compile-time rule ("a share above 1 is not a share") is owed to `graph/validate.ts`; this
    // is the executor's own answer and does not depend on it.
    const declared = sub.budgetShare ?? 1;
    const share = typeof declared === "number" && Number.isFinite(declared) ? Math.min(Math.max(declared, 0), 1) : 0;
  ...
  async #forwardGateDecision(p: RunProjection, w: Wave, childRunId: RunId): Promise<GateRecord | undefined> {
    ...
    // "cross-run READS" enumeration would have hidden. It is safe to retry: the target is
    // re-read, and `idempotencyKey: parent:<taskId>` is derived, so a repeat under a store that
    // has come back neither re-decides nor double-appends.
    let childP: RunProjection | undefined;
    try {
      childP = await this.projection(childRunId);
    } catch (e) {
  ...
  async #endChildRun(childRunId: RunId, reason: string): Promise<void> {
    try {
      ...
      const why = describeThrown(e);
      process.emitWarning(
        `could not stop child run ${String(childRunId)} after the parent finished with it: ${why}; ` +
          `the parent's own decision still stands and the child may be left suspended`,
        { code: "LOOM_CHILD_STOP_FAILED", detail: JSON.stringify({ childRunId, error: why }) },
      );
    }
  }
  ...
  async #summarizeEffect(
    ...
    try {
      ...
      // A CALL THAT DIED STILL RELEASES ITS PROMISE, AND THE RELEASE IS DURABLE — the same
      // pairing `#runAgent`'s turn makes, for the same reason it gives. `projection.ts` folds
      // `budget.reserved` with no matching `budget.settled`, so an orphan here is not memory
      // drift: it is a run that has permanently promised away money it never spent, re-seeded
      // that way by `PolicyEngine.restore` after every restart. Measured before this arm existed,
      // on a compaction whose provider threw with an `error` edge carrying the run past the
      // failed task — `succeeded`, 4 reserved rows against 3 settled, `reservedUsd 0.001856` in a
      // COLD fold. The reservation is appended before the call precisely so a crash cannot hide
      // it; leaving it standing after a failure the process DID see is the same hole with better
      // manners.
      //
      // IN ONE BATCH WITH THE FAILURE, again as the turn does: the two are one fact, and a crash
      // between two appends leaves the fold holding a promise for a call the journal already
      // says died. `actualUsd: 0` is `settle`'s bare-number arm — "this cost nothing" — and not
      // an unknown, because no usage frame ever arrived.
      ctx.policy.settle(reservation, 0);
      const le = toLoomError(e);
      await this.#serialize(ctx.runId, () =>
        ctx.log.append(
          [
            {
              type: "budget.settled",
              payload: { scope, reservedUsd: reservation.amountUsd, actualUsd: 0 },
              actor: SYSTEM_ACTOR("policy"),
              taskId: w.task.taskId,
            },
            { type: "effect.started", payload: { key, kind: "summarize", attempt: 1 }, actor: SYSTEM_ACTOR("context"), taskId: w.task.taskId },
            { type: "effect.failed", payload: { key, error: errorRecord(le) }, actor: SYSTEM_ACTOR("context"), taskId: w.task.taskId },
          ],
          { taskId: w.task.taskId },
        ),
      );
  ...
  async #commit(ctx: RunContext, w: Wave, settling: NodeOutcome): Promise<void> {
    ...
    // loop edge whose bound is spent. Folding a producer's raw choice here and the journaled
...
interface ControlTaint {
  /**
   * The node whose commit made the choice. Named `decidedBy` and not `router` because it is no
   * longer always a router — see `choiceOf` for the four mechanisms that narrow a `take`.
   */
 ...
 * this closes.
 *
 * ## IT IS KEYED ON THE CHOICE, NOT ON THE NODE TYPE, AND THAT IS A CORRECTION
 *
 * This function used to open `if (node.type !== "router") return;`, and a router is only one of
 * the ways a commit's `take` comes out narrower than the graph. Two more were measured through
 * the engine on the same shape — same injected string, same `pay.charge` reading only the clean
 * channel, same de-escalation — with the router DELETED:
 *
 *     two `conditional` edges instead of a router  -> succeeded, gates=0, charged=1
 *     a `function` body returning `take`           -> succeeded, gates=0, charged=1
 *
 * So the question is not "is this a router" but "did this commit pick among edges it could have
 * picked differently among". `choiceOf` answers it.
 *
 * ## WHERE IT LIVES: THE DECIDING NODE'S OWN COMMIT, NOT THE DESCENDANT'S READ
 *
 * The alternative is to ask, at every node's `#decide`, "did some tainted choice select me?" —
 * which walks committed history backwards once per decision and re-derives a fact that was
 * already true and already durable. The choice is a property of the DECIDING NODE: made once,
 * from a channel set the journal records and a `take` the journal records. So it is folded once,
...
/** What one commit picked among, and whether the node itself was what picked. */
interface Choice {
...
/**
 * The edges this commit picked AMONG, and who picked. Empty space when it picked among nothing.
 *
 * ## THE SET THIS COVERS, NAMED
 *
 * Four mechanisms can make `#commit`'s `take` narrower than the node's outbound edges, and
 * they are the three `evaluate` call sites in this file plus `outcome.take`:
 *
 *   1. A `router`. Its space is its DECLARED one — every `cases[].take` plus `fallbackEdge` —
 *      and not "the other outbound edges", because an `error` edge is not an alternative the
 *      router chose between; counting it as one would subtract its downstream from the region
 *      and quietly shrink the guard.
 *   2. A `conditional` edge's `when`, evaluated by `#edgesToTake` for every NON-router source
 *      node against the whole channel scope. The `if (w.node.type === "router") break;` guard
 *      there exists precisely because conditionals are otherwise evaluated for everyone.
 *   3. A `loop` edge's `until`, evaluated by `#loopMayContinue` — from both entrances, the
 *      switch below `#edgesToTake` and a router's `take.filter`.
 *   4. A producer-supplied `take` on a non-router: a `function` or `evaluator` body, a
...
function choiceOf(
  ...
  if (node.type === "router") {
    ...
    if (node.router?.fallbackEdge !== undefined) ids.add(node.router.fallbackEdge as EdgeId);
    const space: EdgeSpec[] = [];
    for (const id of ids) {
      const e = index.edgeById.get(id);
      if (e !== undefined) space.push(e);
    }
...
/**
 * Was this commit's branch decision made from untrusted content? The tainted channels, if so.
 *
 * THREE SOURCES, AND THE THIRD IS WHAT MAKES NESTING HOLD. The deciding node declared a read of
 * a tainted channel; or one of the expressions that narrowed its `take` references one; or the
 * node is ITSELF in an earlier tainted choice's control region, which makes whether it ran at
 * all an attacker's decision and therefore makes its own choice one too. Without the third, two
 * routers in a row launder control flow the way a `function` node used to launder data flow.
 *
 * A NODE'S OWN `reads` ARE EVIDENCE ONLY WHEN THE NODE ITSELF COULD HAVE CHOSEN, and that
 * asymmetry is deliberate. A router evaluates its cases, and a `function` body sees exactly
 * `reads` — for those the read set is what the choice could have been made from. An edge's
 * `when` or `until` is evaluated against `scopeFor` no matter what the SOURCE node declared, so
 ...
 * yet was not in the scope this decision evaluated — `#runRouter` and `#edgesToTake` both read
 * the projection as of the start of the wave — so it cannot have influenced the choice. This is
 * the one place under-approximating is CORRECT rather than convenient, and it is also what keeps
 * the live path and `#restoreEvidence` on one answer: the wave overlay is derived per wave and
 * no fold reproduces it, so reading it here would let a restart mark FEWER nodes than the
 * original process did. Marking fewer is the loosening direction, across a restart, which is the
...
function choiceTainted(
  ...
  for (const e of choice.space) {
    if (e.kind === "conditional" && e.when !== undefined) exprs.push(e.when);
    if (e.kind === "loop" && e.until !== undefined) exprs.push(e.until);
  }
  for (const src of exprs) {
    let refs: readonly string[];
    try {
      refs = referencedChannels(parseExpr(src));
    } catch {
      ...
      // run already holds untrusted content and this condition cannot be read, so the choice
 ...
 * to". True for a forward edge and FALSE for a backward one. A `loop` edge on the arm the
 * router did not take re-enters the graph upstream, so the alternatives set swallowed the
 * deciding node, the taken arm and the irreversible node past it; the subtraction emptied the
 * region and the guard silently did nothing. Measured on one plain `select` graph, driven twice
 * with ONE edge of difference:
 *
 *     no loop edge              -> awaiting_gate, gates=1, charged=0
 ...
 *
 * `followLoops` false drops `loop` edges as SEEDS as well as in the walk. Dropping them only in
 * the walk would still let a `loop` seed hand over its upstream target and reach everything from
 * there, which is the whole of the bypass `controlRegion` describes.
 */
function reachableFromEdges(index: GraphIndex, seeds: Iterable<EdgeId>, followLoops: boolean): Set<NodeId> {
  const seen = new Set<NodeId>();
  const queue: NodeId[] = [];
  ...
  const traversable = (kind: EdgeSpec["kind"]): boolean =>
    kind !== "compensation" && (followLoops || kind !== "loop");
  for (const id of seeds) {
    const e = index.edgeById.get(id);
    if (e !== undefined && traversable(e.kind)) push(e.to);
  }
  for (let i = 0; i < queue.length; i++) {
    for (const e of index.outbound.get(queue[i]!) ?? []) {
      if (traversable(e.kind)) push(e.to);
    }
  }
  return seen;
}
...
/**
 * The compiled floors as a durable row: `(nodeId, posture)` sorted by node id.
 *
