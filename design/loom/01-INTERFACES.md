...
| `E_FUNCTION_UNAVAILABLE` | unavailable | `GraphExecutor`, on a body's behalf | A `function` or `evaluator{assertion}` body returned `{ retry: { reason } }`. Retryable by class, which is the only route from a sandboxed body to `NodeSpec.retry`: every throw out of the realm normalizes to `internal` |
...
| Property | Contract |
|---|---|
| Errors | `E_GATE_NOT_FOUND`, `E_GATE_ALREADY_RESOLVED`, `E_GATE_NOT_AUTHORIZED` (also from `raise`, for a `defaultAction` the gate's own `allowEdit` forbids), `E_HUMAN_APPROVAL_REQUIRED` — **a decision the union does not name, and a rejection with no reason; both from `resolve`, `resolveBatch` and from `raise` for a `defaultAction`**. `E_GATE_EXPIRED` is not thrown — it is the code an expiry writes into `run.failed`. **`E_GRAPH_MISMATCH`** — the graph bound to the run is not the one it compiled, thrown before any decision that CONTINUES the run (`approve`, `edit`, `redirect`) and deliberately **not** before `reject`, which needs no graph and is an operator's exit when one has drifted |
| Graph binding | A continuing decision is refused unless three things match what `run.compiled` and `run.started` recorded: the spec (`graphHash`), the RESOURCES its refs resolved to (`resolutionManifest` — the hash covers the pointers, never the bytes behind them), and the compiled oversight FLOOR (`plans` are excluded from the hash, so a process registering fewer tools recomputes a weaker posture under an identical hash). Reproduced before it existed: `loom approve <run> <gate> --graph OTHER.json` ran the other graph |
