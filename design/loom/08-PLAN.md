...
| **R4** | **Determinism leaks** — a `function` resource calls `Date.now()`/`Math.random()`/`fetch` directly and replay diverges | **high** | medium | A body runs in a `vm` context whose global set omits `Date`, `fetch` and everything else host-side, so the clock and the network are unreachable by accident; GRAPH013 refuses clock-dependent expressions; a CI job replays every fixture and asserts state-hash equality. **The mitigation is narrower than the risk, and this row used to hide that** by naming two APIs that do not exist: the effect boundary is inside `Engine` and nothing on a node body's context declares one, so *there is no sanctioned path for a body to record its own nondeterminism*. `ctx.now()` is injected and host-controllable but **not** journaled; `Math` is passed through whole, so `Math.random()` runs unrecorded and there is no seeded PRNG; there is no publish-time lint. A body that must replay identically reads its timestamp from a channel | Replay CI fails |
...
| 6 | **Durable human gate** | `kill -9` the process while the gate is open; restart; the gate is in the queue **with its SLA clock intact**; approving resumes the run. **Two halves, and they are proven by two different tests — do not read one as covering the other.** ① *Durability, proven by a process that really was killed:* `test/run/restart-crash.test.ts` forks `restart-crash.child.ts`, waits for its `gated` message so the kill is ordered by an event and not a timer, `SIGKILL`s it, asserts the exit *signal* was `SIGKILL`, and asserts `journal.db-wal` and `journal.db-shm` are **still on disk and the `-wal` non-empty** — which is what separates this from `close()`, since a clean close checkpoints the WAL and deletes both. It then opens a second store in a genuinely new process, recovers `awaiting_gate` with the same open `gateId`, approves, and asserts the deferred `fs.write` ran exactly once. A sibling test does the clean close as the control, so the `-wal` assertion is pinning something. ② *The SLA clock is **not** in that test and cannot be:* the skeleton's `approve` node is `humanGate: { ref: "oversight/demo-write@stable" }` with no `sla` block (`test/run/skeleton.ts`), so there is no deadline for the crash to preserve — `grep -aic sla` over both crash files returns 0. What carries the clock across a restart is `test/run/gate-clock.test.ts`'s *"THE SWEEP REACHES A RUN THIS PROCESS NEVER ATTACHED"*: a **new `Engine` over the same store**, never attached to the run, advances an injected clock past `respondWithinMs` and the sweep still fires that gate — because deadlines are absolute timestamps folded out of the journal, not in-memory timers. That is a fresh engine, not a killed process, and it is on a `MemoryStateStore`. The two together are the row; neither alone is |
