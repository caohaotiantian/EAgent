...
| Commits on `loom` | 145, **31 unpushed** | `git log --oneline origin/loom..HEAD \| wc -l` |
...
| **D2** | **`rewind`, the only recovery command, destroys the run** — and the boundary a user would naturally pick is the wrong kind | Rewinding to a node's own declared `checkpoint.created` seq leaves the task `leased` forever and a later `advance` fails `E_OUTPUT_MISSING`. Only a `task.ready` seq works, and `rewind` never self-advances. `04-OVERSIGHT.md` describes `rollback{to,rewind}` as working; README omits it |
...
| **D6** | **README's front page claims declarative fallback chains; nothing constructs one** | `FallbackAdapter` is defined and fully tested; `grep -ran 'new FallbackAdapter' packages/core/src/` returns nothing. The models-file route rows accept `{adapter, model}` only. Plan written at `.agent/fallback-wiring/plan.md`; ~20 lines in `readModels` |
| **D7** | **`EdgeSpec.codes` — the error-edge code filter — has zero readers**, so every error edge is a catch-all whatever it declares | `#errorEdges` filters `e.kind === "error"` and nothing else. Asymmetric with `RetryPolicy.onlyIf`, which IS read, so an author who learned code-filtering works for retry reasonably assumes it works here |
...
**Pushing.** `origin/loom` exists and the branch is published, but **31 commits are unpushed**
