...
| 7 | Local → distributed changes implementations, never call sites | PASS in design (G3) | **PARTIAL** | One conformance suite passes against both `MemoryStateStore` and `SqliteStateStore`, and another against `InProcessScheduler` and `LeasedScheduler` — the executor takes a `Scheduler`, so the swap is a constructor argument. **What the swap does not yet buy:** `Engine.advance` returns or finishes when the ready set is empty and only THEN consults `#scheduler.select`, so `LeasedScheduler`'s reclaim of an EXPIRED lease — the one thing `InProcessScheduler` cannot do, and by construction never `ready` — is unreachable through the executor. It is exercised only by calling `select` directly, and `LeasedScheduler`'s own docstring concedes "not wired into the v1 executor". Partition assignment is still G3 and still deferred |
...
| L1 gate surfacing / routing / escalation | **PROVEN** | `run/delivery.ts`: injected channels, a zero-dep `WebhookChannel` on global `fetch`, a `ConsoleChannel` fallback that cannot fail, per-channel failure journaling, and a tiered escalation chain whose clock resets per tier — `test/run/delivery.test.ts`, including four separate ways delivery failure could have leaked into an approval. The return path is `SignedWebhookChannel.parseCallback` + `GateCallbackRouter` + `POST /runs/:id/callbacks/:channel` — `node --test packages/core/test/run/callback.test.ts`, against a real gated run — plus a socket-level pair in `test/server/http.test.ts` that posts a signed callback with no bearer (200, action ran) and a forged one (403, gate open, nothing journaled). **The wiring closed 2026-08, and all three halves of the old limit are now false.** `HumanGateNode.delivery` is a compiled field (`graph/spec.ts`), shape-checked by `checkDelivery` and cross-checked against `onTimeout: "escalate"`; `Engine` carries it onto the raise through `scheduleOf`; `bin/loom` builds a `GateDispatcher` from `--channels-file` and opens the callback route when a channel is answerable; and `serve` ticks `sweepTimeouts` every second by default through `Engine.sweepGates` → `GateSweeper.sweep`. A restarted process re-arms each gate it re-attaches (`Engine.rehydrateGates`), so a tier survives a redeploy. **What is still absent:** a mirror gate on a `subgraph` node inherits no delivery spec, and the sweeper sees only the `limit` most recent runs |
...
| L5 retention tiering | **PROVEN** | `journal/retention.ts`: tiers by age, a WORM audit tier with its own window, and `TierManager.archive` writing both. `test/journal/retention.test.ts`, incl. replaying a run rebuilt from cold storage alone and proving a cold-retention cut leaves the approval record intact. **No caller in `src/`:** `TierManager` is exported from `@loom/core` and constructed only by its own test — nothing in `cli.ts`, `server/` or `run/` archives — so a `bin/loom` deployment's journal never leaves the hot tier. PROVEN is right under this file's legend ("a test asserts it") and incomplete without this line, which is the same disclosure the L1 row above used to carry |
...
| 3.1 durable suspension across restart | **PROVEN** | `test/run/restart-crash.test.ts` (landed 2026-08-15; 3 pass / 0 fail in 324 ms). It forks `test/run/restart-crash.child.ts`, waits for the child's `gated` IPC message so the kill is ordered by an event and not a timer, `SIGKILL`s it, and asserts `how.signal === "SIGKILL"` — killed, not asked to stop. Then the three assertions that make it a crash rather than a shutdown: `journal.db-wal` and `journal.db-shm` are **still on disk**, and the `-wal` is **non-empty** (existence alone would not rule out a checkpoint that left the file behind). A second `SqliteStateStore` in a genuinely new process then `attach`es, recovers `awaiting_gate` with the **same** open `gateId`, approves, and asserts the deferred `fs.write` ran exactly once. A sibling test performs the clean `close()` as the control, so the `-wal` assertion is pinning something rather than restating a default. This cell previously read "**No such test exists**" and pointed at REGISTER D12; the test now exists, the pointer is stale, and `docs-drift.test.ts`'s "EVERY TEST FILE THE DoD CITES AS EVIDENCE EXISTS" check pins the path above from here on. **The narrower claims still hold and are still separate:** `test/run/skeleton.test.ts` row 6 proves the same recovery across a *clean* close, and `test/run/oversight-survives-restart.test.ts` proves it for the escalation ceiling and the accumulated spend. **Not proven by this test: the SLA clock.** The skeleton's `approve` node declares no `sla`, so there is no deadline for the crash to preserve — see 08-PLAN acceptance row 6, which now splits that half out |
