...
echo "  candidate was. KEEP --proposed-by, naming whoever produced the candidate (step 5 passes"
echo "  --proposed-by maintainer): omitting it SKIPS checks 9 and 10 by omission, which is the"
echo "  permissive default the gate refuses everywhere else and the audit's"
echo "  lineage-check-off-by-omission finding. A suite that carries generatedBy is a generated"
echo "  suite, and a generated suite needs a named proposer to compare against."
echo "  This replayed door grades work channels and never the grader; 12-grader-unchanged"
echo "  refuses a candidate that swaps its evaluator. To measure an IMPROVEMENT, attest the"
echo "  operator's exam and judge live:"
echo "    $LOOM exam attest $WS/exams/review-bench-exam.json --cohort <a review-bench run> --as <you> --workspace $WS"
echo "    $LOOM promote $WS/candidates/review-bench-v2.json --against-cohort <that run> --models-file $MODELS --workspace $WS"
