...
hand the arm the same fact the live run did. `truncated` and `bytes` are served on a SUCCESS:
`truncated` by `fs.read`, `net.fetch`, `proc.exec`, `fs.glob` and `fs.grep` (the tools that cap
what they return), and `bytes` by `fs.read` and `net.fetch` (the whole source's size) and
`fs.write` and `fs.edit` (what landed) — so a whole read is `{ok: true, truncated: false, bytes}`,
a listing `{ok: true, truncated: false}`, a write `{ok: true, bytes}`. That is how `grant-prior.js` and `grant-weigh.js` refuse a cut document
