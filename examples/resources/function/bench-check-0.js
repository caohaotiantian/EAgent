// Case 0 alone. SIX EVALUATOR NODES, NOT ONE, because `readSignals` computes S1 as
...
(view) => {
  const c = (view.get("cases") ?? [])[0];
  const v = (view.get("verdicts") ?? [])[0];
  if (c === undefined) return { writes: { verdict0: { pass: false, score: 0, confidence: 1, detail: "no case 0 in this input" } } };
  ...
  return { writes: { verdict0: { pass: correct, score: correct ? 1 : 0, confidence: 1, detail: detail } } };
}
