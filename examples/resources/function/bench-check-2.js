// Case 2 alone. SIX EVALUATOR NODES, NOT ONE, because `readSignals` computes S1 as
...
(view) => {
  const c = (view.get("cases") ?? [])[2];
  const v = (view.get("verdicts") ?? [])[2];
  if (c === undefined) return { writes: { verdict2: { pass: false, score: 0, confidence: 1, detail: "no case 2 in this input" } } };
  ...
  return { writes: { verdict2: { pass: correct, score: correct ? 1 : 0, confidence: 1, detail: detail } } };
}
