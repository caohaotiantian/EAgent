// Case 4 alone. SIX EVALUATOR NODES, NOT ONE, because `readSignals` computes S1 as
...
(view) => {
  const c = (view.get("cases") ?? [])[4];
  const v = (view.get("verdicts") ?? [])[4];
  if (c === undefined) return { writes: { verdict4: { pass: false, score: 0, confidence: 1, detail: "no case 4 in this input" } } };
  ...
  return { writes: { verdict4: { pass: correct, score: correct ? 1 : 0, confidence: 1, detail: detail } } };
}
