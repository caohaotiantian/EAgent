// Case 3 alone. SIX EVALUATOR NODES, NOT ONE, because `readSignals` computes S1 as
...
(view) => {
  const c = (view.get("cases") ?? [])[3];
  const v = (view.get("verdicts") ?? [])[3];
  if (c === undefined) return { writes: { verdict3: { pass: false, score: 0, confidence: 1, detail: "no case 3 in this input" } } };
  ...
  return { writes: { verdict3: { pass: correct, score: correct ? 1 : 0, confidence: 1, detail: detail } } };
}
