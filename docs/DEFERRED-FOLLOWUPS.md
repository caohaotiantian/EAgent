...
| RW7c-4 | **Histogram metrics** for otel-exporter (operation/run-duration buckets, `gen_ai.client.operation.duration`) — v1 ships Sum counters only. | `docs/design/2026-06-30-otlp-metrics-logs.md` (§3) | M · L | Explicit-bucket histograms need bucket-boundary config + a heavier data-point shape; the high-value token/tool Sums shipped first. |
...
share — no current consumer). Larger designs remain correctly deferred: **RW8a-3** (GoT extensions — DSL/multi-round/cross-tool; RW8a-1's ToT+GoT both shipped 2026-06-30), **RW7b-1**
