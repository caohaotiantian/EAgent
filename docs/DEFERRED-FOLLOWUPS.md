...
| SRV-4 | External stream inputs read with no size cap (`parseSSE` buffer, provider accumulators, MCP `res.text()`/readline) → OOM/DoS; `readCapped` primitive exists but isn't applied. | `providers/http.ts:29`; `mcp.ts:189,354` | LOW | **RESOLVED (parseSSE)** (Wave 3, `docs/design/2026-07-02-sse-buffer-cap.md`) — the shared `parseSSE` incomplete-event buffer is capped at `EAGENT_MAX_SSE_EVENT_BYTES` (default 16 MiB; `maxSseEventBytes` helper `providers/http.ts:16-21`, enforced in `parseSSE` at `:59-61`), protecting all 3 fetch providers. **SRV-4b (deferred):** the MCP transport reads (`res.text()`, stdio readline) — a distinct mechanism, MCP separately gated. |
