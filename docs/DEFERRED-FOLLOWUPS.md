...
| SRV-4 | External stream inputs read with no size cap (`parseSSE` buffer, provider accumulators, MCP `res.text()`/readline) → OOM/DoS; `readCapped` primitive exists but isn't applied. | `providers/http.ts:29`; `mcp.ts:189,354` | LOW | **RESOLVED (parseSSE)** (Wave 3, `docs/design/2026-07-02-sse-buffer-cap.md`) — the shared `parseSSE` incomplete-event buffer is capped at `EAGENT_MAX_SSE_EVENT_BYTES` (default 16 MiB; `maxSseEventBytes` helper `providers/http.ts:16-21`, enforced in `parseSSE` at `:59-61`), protecting all 3 fetch providers. **SRV-4b (RESOLVED 2026-07-02, `docs/design/2026-07-02-mcp-read-caps.md`):** `readCapped` relocated to `src/extensions/lib/read-capped.ts`; the HTTP transport caps both SSE/JSON reads via `maxMcpReadBytes()` (`EAGENT_MAX_MCP_READ_BYTES`, default 16 MiB) + throw-on-overflow, and the stdio transport uses a byte-bounded `createBoundedLineReader` (discard-to-newline) in place of unbounded `createInterface`. |
...
**Known test-coverage caveat (env-conditional, 2026-07-02 inventory):** `test/codeact.test.ts:121`
silently skips the **python3 interpreter-dispatch** assertion on any host without `python3` on `PATH`
(the JS dispatch path is unconditionally covered). Low impact; noted for completeness. The real-launcher
confinement + real self-improve eval are likewise CI-job-gated (the `sandbox-linux` bwrap job), not the
default `npm test` — see the RW8b-1 / RW9.3-1 / RW6c-3 rows.
