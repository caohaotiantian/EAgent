...
| RW8a-1 | **GoT (Graph-of-Thought) operations** in `reasoning-search` — thought *aggregation* (merge several thoughts) + *refinement* (improve a thought in place), i.e. a thought DAG. (ToT multi-step beam search **SHIPPED** 2026-06-30 as `tree_search`.) | `docs/design/2026-06-30-tree-search.md` (KDD-2, §3) | M–H · M | GoT's aggregate/refine operators need a thought-DAG + merge prompts + dependency tracking — a much larger surface; the shipped ToT beam search proves the expand→score→prune spine GoT would reuse. |
