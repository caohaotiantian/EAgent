...
`D.18` the mailbox. Two came off this list 2026-08-28: `D.10` the `preAuthorization` envelope,
answered by permanent refusal rather than by a build, and `D.15` quorum and delegation, answered
by measuring that quorum already worked and deleting the declarations that claimed otherwise.
...
| `B.7` **Quorum, delegation and trust-tier approvals** — delibera | **CLOSED 2026-08-28 by deletion** | The framing was wrong, and measuring it is what decided the item: quorum was never missing. `join{mode:"quorum", k}` over N `human_gate` nodes does k-of-n today — measured with three gates, one approval leaves `writes=0` and the second fires the guarded write — so `ApprovalSpec.mode`, `.k`, `.delegation` and `DelegationSpec` were DELETED rather than implemented, along with their three bespoke refusals. `tiered` was not implementable from its own declaration because no field anywhere defines a tier; delegation's refusal was itself defeatable (`delegation: {allowd: true}` compiled clean). `examples/graphs/two-person-approval.json` ships the composition with a test that drives it. See §D.15. |
...
| `D.15` quorum and delegation | **ANSWERED 2026-08-28 — QUORUM ALREADY EXISTED; the declarations are deleted** | `ApprovalSpec.mode`/`.k`/`.delegation` and `DelegationSpec` removed from `graph/spec.ts` (kernel #3); three `unsupported(...)` refusals, the `delegation.allowed` type check and `GRAPH014_APPROVAL_UNSUPPORTED` removed from `validate.ts`; `mode: "single"` stripped from every file that carried it. **Landed after D.5-a, which is the whole safety argument:** without D.5-a's `NESTED_FIELDS.approval` check, this deletion would make `mode: "quorum"` compile CLEAN and run as one-approver — the exact failure the block exists to prevent, caused by the change meant to prevent it. After it, `GRAPH020_UNKNOWN_FIELD` catches `mode`, `k` and `delegation` AND `modee`, `quorumK` and `delegate`, where three exact strings were caught before. Breaking removal: `DelegationSpec` leaves the pinned surface. Residue, in the example's own `labels` and in its test: a short-circuiting quorum join leaves the unneeded gates OPEN — `JoinNode`'s straggler-cancellation gap — so the run is still `awaiting_gate` after the write fires. |
...
the subgraph span · `run.cancelled.forced` · the operator surface ·
...
answered by deleting the field), the `preAuthorization` envelope (§D.10, answered by refusing it
permanently), and quorum and delegation (§D.15, answered by measuring that quorum already worked
in `join`).)
